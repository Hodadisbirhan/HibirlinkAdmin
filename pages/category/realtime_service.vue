<template>
  <div class="relative">
    ...
    >
      <div
        class="p-4 pb-0 relative bg-light1500/20 border hover:border-primary rounded-md shadow-sm"
      ...
      >
        ...
        <div class="flex justify-between py-1 items-center">
          <p class="font-semibold text-lg text-green400">{{ cat.en }}</p>
...
import updateMainCategoryGql from "@/apollo/mutation/service/physical/updateMainCategory.gql";
import deleteCategoryGql from "@/apollo/mutation/product/physical/deleteCategory.gql";
import addCategoryGql from "@/apollo/mutation/service/physical/addCategory.gql";
definePageMeta({
  layout: "custom",
});
...
const actionSelect = (value) => {
  if (value.action == "Edit") {
    ...
    categoryListItems.value.find((elem) => {
      return elem.id == value.id.id;
    })?.lng_id;
...
const updateCategory = async (value) => {
  ...
  console.log({ id, logo, en_name, am_name, cat_id });
