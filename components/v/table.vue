<template>
  <div :class="supporterClass" class="text-sm my-2 h-full overflow-x-auto">
    <table
      class="border-collapse w-full rounded font-body min-w-[1000px]"
      :class="[tableClass]"
    >
      <thead
      ...
      <tbody v-if="loading">
        <tr class="odd:bg-green400/5 text-gray" :class="[trClass]">
      ...
      <tbody v-else :class="[tbodyClass]">
        <tr
          ...
          class="odd:bg-light200/40 text-gray"
        >
          <td v-if="isChecked">
            <span :class="[bodyClass]" class="flex items-center justify-center">
              <input
                type="checkbox"
                name="row"
